...
<style>
    ...
    /* Style the radio buttons in the sidebar to look like navigation links */
    .stRadio > div {
        gap: 1rem;
    }
    .stRadio > div > label {
    ...
    /* Style Chat Input to match the sleek design */
...
try:
    history_res = requests.get(f"{API_BASE_URL}/documents/history")
...
# Set default index
default_idx = 0
if st.session_state.document_filename in kb_options:
...
# Update session state if changed via dropdown
...
if page == "Ingestion":
    ...
    with col_upload:
        ...
        if uploaded_file is not None and uploaded_file.name != st.session_state.document_filename:
            ...
            with st.spinner(f"Uploading {uploaded_file.name}..."):
                ...
                if res.status_code == 200:
                    st.success(f"Upload successful! Processing started.")
                    ...
                    while retries < max_retries:
                        status_res = requests.get(f"{API_BASE_URL}/documents/status/{uploaded_file.name}")
                        if status_res.status_code == 200:
                            status = status_res.json().get("status")
                            if status == "completed":
                                status_placeholder.success("Graph extraction complete!")
                                st.session_state.graph_ready = True
                                st.rerun() # Refresh page to update history
                                break
        ...
        if st.session_state.graph_ready:
            st.markdown("---")
            st.markdown(f"**CURRENT EXTRACTION PROGRESS**")
            st.markdown(f"**{st.session_state.document_filename}**")
            st.progress(100)
            
            # Mock Metric Cards to match UI
        ...
        try:
            ...
            if history_res.status_code == 200:
                ...
                for item in history:
                    ...
                    bg_color = "#F1F5F9" # Gray for processing
                    border_color = "#CBD5E1"
...
# --- View: Knowledge Graph ---
    ...
    if not st.session_state.document_filename or st.session_state.document_filename == "Global (All Documents)":
        st.warning("Please select a specific document from the sidebar to visualize its Knowledge Graph. Rendering the global graph is too large for the browser.")
    else:
        ...
        with st.spinner("Loading graph data..."):
            graph_res = requests.get(f"{API_BASE_URL}/graph/visualize/{st.session_state.document_filename}")
        ...
        if graph_res.status_code == 200:
            ...
            with col_graph:
                st.markdown("<div style='border: 1px solid #CBD5E1; border-radius: 0.5rem; padding: 0.5rem; background-color: white;'>", unsafe_allow_html=True)
                if nodes:
                    clicked_node_id = agraph(nodes=nodes, edges=edges, config=config)
                else:
                    st.info("No entities were extracted for this document.")
...
# --- View: Chat / Q&A ---
    ...
    if not st.session_state.document_filename:
        st.title("Chat & Q&A")
        st.warning("Please upload and process a document in the 'Ingestion' tab first.")
    else:
        ...
        with col_recent:
            ...
            if not user_queries:
                ...
                for q in reversed(user_queries[-5:]): # Show last 5
                    st.markdown(f"""
        ...
        with col_chat:
            st.markdown("<div style='height: 600px; overflow-y: auto; padding-right: 1rem; padding-top: 1rem;'>", unsafe_allow_html=True)
            ...
            # Dummy toggles for UI matching
            st.markdown("""
            <div style='display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 1rem; background-color: white; border-radius: 1rem; margin-top: 0.5rem; border: 1px solid #E2E8F0;'>
                <div style='display: flex; gap: 1rem;'>
                    <span style='font-size: 0.75rem; font-weight: bold; color: #1E293B;'><input type='checkbox' checked disabled/> ENTITY EXTRACTION</span>
                    <span style='font-size: 0.75rem; font-weight: bold; color: #94A3B8;'><input type='checkbox' disabled/> CROSS-GRAPH REASONING</span>
                </div>
                <div style='font-size: 0.7rem; color: #94A3B8;'>Current model: <strong>Graph-LLM v2-Turbo</strong></div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # Handle the LLM call outside the static render to avoid UI jumpiness
